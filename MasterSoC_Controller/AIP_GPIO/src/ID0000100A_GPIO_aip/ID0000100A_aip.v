`define ID0000100A_MEM_IN 1

`define ID0000100A_MEM_OUT 2

`define ID0000100A_CONF_REG 1


// `define ID0000100A_STREAMING_IN
// `define ID0000100A_STREAMING_OUT

module ID0000100A_aip
(
  clk,
  rst,
  en,
...
(
  ...
  //--- IP-core ---//
  `ifdef ID0000100A_MEM_IN
  rdDataMemIn,
  rdAddrMemIn,
  `elsif ID0000100A_STREAMING_IN
  rdDataMemIn,
  rdAddrMemIn,
  `endif

  `ifdef ID0000100A_MEM_OUT
  wrDataMemOut,
  wrAddrMemOut,
  wrEnMemOut,
  `elsif ID0000100A_STREAMING_OUT
  wrDataMemOut,
  wrEnMemOut,
  `endif

  `ifdef ID0000100A_CONF_REG
  rdDataConfigReg,
  `endif

  `ifdef ID0000100A_STREAMING_IN
  dataInStreaming,
  startStreaming,
  wrInStreaming,
  `endif

  `ifdef ID0000100A_STREAMING_OUT
  dataOutStreaming,
  doneStreaming,
  wrOutStreaming,
  `endif
  ...
  `ifdef ID0000100A_MEM_IN
  localparam [(`ID0000100A_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] MEM_IN_ADDR_WIDTH = { 16'd1 };

  localparam [((`ID0000100A_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = { 5'b00001, 5'b00000 };
  `endif

  `ifdef ID0000100A_MEM_OUT
  localparam [(`ID0000100A_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] MEM_OUT_ADDR_WIDTH = { 16'd1, 16'd1 };

  localparam [((`ID0000100A_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = {5'b00101, 5'b00100 , 5'b00011, 5'b00010 };
  `endif

  `ifdef ID0000100A_CONF_REG
  localparam [(`ID0000100A_CONF_REG*CONF_REG_ADDR_MAX_WIDTH)-1:0] CONF_REG_ADDR_WIDTH = { 3'd4 };
  ...
  `ifdef ID0000100A_MEM_IN
  output [(`ID0000100A_MEM_IN*DATA_WIDTH)-1:0] rdDataMemIn;
  input wire [(`ID0000100A_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemIn;

  wire [(`ID0000100A_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  wire [`ID0000100A_MEM_IN-1:0] wrEnMemIn;

  wire [DATA_WIDTH-1:0] rdDataMemIn0;
  `elsif ID0000100A_STREAMING_IN
  output [DATA_WIDTH-1:0] rdDataMemIn;
  input wire [MEM_ADDR_MAX_WIDTH-1:0] rdAddrMemIn;
  `endif // ID0000100A_MEM_IN

  `ifdef ID0000100A_MEM_OUT
  input [(`ID0000100A_MEM_OUT*DATA_WIDTH)-1:0] wrDataMemOut;
  input [(`ID0000100A_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemOut;
  input [`ID0000100A_MEM_OUT-1:0] wrEnMemOut;

  wire [(`ID0000100A_MEM_OUT*DATA_WIDTH)-1:0] rdDataMemOut;
  wire [(`ID0000100A_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;
  `elsif ID0000100A_STREAMING_OUT
  input [DATA_WIDTH-1:0] wrDataMemOut;
  input [0:0] wrEnMemOut;
  `endif // ID0000100A_MEM_OUT
  ...
  `ifdef ID0000100A_MEM_IN
  generate
      for(i=0; i<`ID0000100A_MEM_IN; i = i + 1) begin :  MEMIN
          if (0 == i)
  ...
  `ifdef ID0000100A_MEM_OUT
  generate
      for(i=0; i<`ID0000100A_MEM_OUT; i = i+1) begin : MEMOUT
          simple_dual_port_ram_single_clk
          #(
              .DATA_WIDTH (DATA_WIDTH),
...
(
  ...
  `ifdef ID0000100A_MEM_IN
  parameter [((`ID0000100A_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = {5'b00011, 5'b00010, 5'b00001, 5'b00000};
  `endif

  `ifdef ID0000100A_MEM_OUT
  parameter [((`ID0000100A_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = {5'b00101, 5'b00100};
  `endif
  ...
  `ifdef ID0000100A_MEM_IN
  output wire [(`ID0000100A_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  output wire [`ID0000100A_MEM_IN-1:0] wrEnMemIn;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regWrAddrMemIn [0:`ID0000100A_MEM_IN];
  `endif

  `ifdef ID0000100A_MEM_OUT
  output wire [(`ID0000100A_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regRdAddrMemOut [0:`ID0000100A_MEM_OUT];
  `endif
  ...
  `ifdef ID0000100A_MEM_IN
  generate
      for (indexEn=0; indexEn<`ID0000100A_MEM_IN; indexEn= indexEn + 1) begin : WR_EN_MEMIN
          assign wrEnMemIn[indexEn] = writeAIP & (configAIP == CONFIG_MEM_IN[CONFIG_WIDTH*(indexEn*2)+:CONFIG_WIDTH]);
      end // for ID0000100A_MEM_IN
   endgenerate
  `endif
  ...
  always @(posedge clk or negedge rst) begin
    if (!rst) begin
      `ifdef ID0000100A_MEM_IN
      for (i=0; i<`ID0000100A_MEM_IN; i= i + 1) begin
        regWrAddrMemIn[i] <= 'd0;
      end
      `endif

      `ifdef ID0000100A_MEM_OUT
      for (i=0; i<`ID0000100A_MEM_OUT; i= i + 1) begin
  ...
  `ifdef ID0000100A_MEM_IN
  generate
    for (indexAddr=0; indexAddr<(`ID0000100A_MEM_IN); indexAddr=indexAddr+1) begin: MEM_IN_ADDR
  ...
  `ifdef ID0000100A_MEM_OUT
  generate
    for (indexAddr=0; indexAddr<(`ID0000100A_MEM_OUT); indexAddr=indexAddr+1) begin: MEM_OUT_ADDR
